...
var Application = function () {
	...
	var getLineInfo = function (fileType, fileName, syntax, line_x, documentLines, cacheLine) {
		...
		for (var char_x in line) {
			...
			if (cache.match_lock == "" || cache.match_lock == "in_single_quote_str") {
				if (fileType != 'php' || cache.php_syntax != "html" || cache.html_open_tag == true) {
					if (char == "'" && (prev_char != "\\" || prev2_char == "\\")) {
						match_length = 1;
			...
			if (cache.match_lock == "" || cache.match_lock == "in_double_quote_str") {
				if (fileType != 'php' || cache.php_syntax != "html" || is_true(cache.html_open_tag) == true) {
					if (char == '"' && (prev_char != "\\" || prev2_char == "\\")) {
						match_length = 1;
	...
	var cacheHtmlElements = function (fileName, line_x) {
		...
		if (self.documents[fileName]['lines'][line_x]['syntax'] == "html") {
			...
			try {
				...
				for (let line_x in self.documents[fileName]['line_links']) {
					if (self.documents[fileName]['line_links'][line_x] == lineNumber) {
						delete self.documents[fileName]['line_links'][line_x];
					}
				}

				while (line_x > lineNumber) {

					self.documents[fileName]['line_links'][line_x] = lineNumber;
	...
	var cacheDocumentLines = function () {
		...
		self.documents[fileName]['line_links'] = {};
		...
		for (var line_x in lines) {
			try {
				...
				self.documents[fileName]['lineIds'][self.documents[fileName]['lines'][line_x]['id']] = line_x;
			} catch (e) {
				console.log(e)
			}
		}
	...
	var updateCacheDocumentLines = function (startLine, endLine) {
		...
		var updateLinkedLines = function (line_x) {
			...
			let line_links = self.documents[fileName]['line_links'];
			if (isset(line_links[line_x])) {
				let linkLineNum = line_links[line_x];
				changedLines[linkLineNum] = true;
				self.documents[fileName]['lines'][linkLineNum] = getLineInfo(fileType, fileName, syntax, linkLineNum, lines);
				return linkLineNum;
			}
			return null;
		}
		...
		try {
			...
			if (diff == 0 && startLine == endLine) {
				...
				for (var line_x in lines) {
					...
					if (line_x < startLine) {
						...
						line_ids[self.documents[fileName]['lines'][line_x]['id']] = line_x;
					} else if (diff < 0) {
						if (line_x <= (endLine + diff)) {
							let lineNum = updateLinkedLines(line_x);
							if (lineNum !== null) {
								line_ids[self.documents[fileName]['lines'][lineNum]['id']] = lineNum;
							}
							self.documents[fileName]['lines'][line_x] = getLineInfo(fileType, fileName, syntax, line_x, lines);
							line_ids[self.documents[fileName]['lines'][line_x]['id']] = line_x;
				...
				self.documents[fileName]['lineIds'] = line_ids;
			}
			...
			//console.log(self.documents[fileName]);
		} catch (e) {
			console.log(e);
			return false;
		}
