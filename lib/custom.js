...
var Application = function () {
	...
	this.editorCreateDecoration = function (key, decoration) {
		...
		for (var fileName in self.documents) {
			self.documents[fileName]['cacheEnabled'] = true; //caching enabled by default
			self.documents[fileName]['decorationEnabled'] = true;
	...
	let documentCacheEnabledParams = {
		lineCount: null
	};
	let elementVoids = {
		area: true,
	...
	var shiftCacheLineValues = function (fileName, line_x) {
		...
		if (self.documents[fileName]['decorationEnabled'] == true) {
			if (diff !== 0) {
	...
	var getLineInfo = function (fileType, fileName, syntax, line_x, documentLines, cacheLine) {
		...
		if (cacheDocumentLines && is_true(self.fullCache) != true) {
		...
		for (var char_x in line) {
			...
			if (fileType == 'php') {
				if (cache.php_open == false && line[char_x] == '<' && (/<\?/).test(line.substring(char_x, char_x + 2))) {
					cache.php_open = true;
					php_open_line = true;
					if (cache.match_lock != "") {
				...
				if (cache.php_open == true && line[char_x] == '?' && (/\?>/).test(line.substring(char_x, char_x + 2))) {
			...
			if (cache.match_lock == "" || cache.match_lock == "in_comment_line") {
				if (cache.match_lock == "" && line[char_x] == '/' && prev_char != "\\" && (/\/\//).test(line.substring(char_x, char_x + 2))) {
					match_length = 2;
			...
			if (cache.match_lock == "" || cache.match_lock == "in_comment_block" || cache.match_lock == "in_comment_block_end") {
				...
				if (cache.match_lock == "in_comment_block" && line[char_x] == '*' && (/\*\//).test(line.substring(char_x, char_x + 2))) {
			...
			if (cache.match_lock == "" || cache.match_lock == "in_reg_str") {
				if (cache.php_syntax != "html" || cache.html_open_tag == true) {
					if (cache.match_lock == "" && char == "/") {
						...
						let isValid = false;
						debug('getLineInfo: while (regClosePos > -1)', line_x)
						while (regClosePos > -1) {
						...
						debug('getLineInfo: while (regClosePos > -1) END', line_x)
						if (isValid) {
			...
			if (cache.php_open == false && (fileType == 'html' || fileType == 'php')) {
				...
				if (cache.html_open_tag == false && (cache.php_syntax === '' || cache.php_syntax === 'html') && /</.test(line[char_x]) && /[a-zA-Z]/.test(line[char_x + 1])) {
					cache.php_syntax = 'html'
					cache.html_open_tag = true;
					html_open_tag_start = true;
					cache.html_in_tag = true;
				}
				if ((cache.php_syntax === '' || cache.php_syntax === 'html') && line[char_x] == "<" && (/<!--/).test(line.substring(char_x, char_x + 4))) {
					cache.php_syntax = 'html'
		...
		if (syntax == 'php') {
			...
			if (textFormatted.indexOf("/*") > -1) {
				...
				var x = line_x + 1;
				debug('getLineInfo: while (x < documentLines.length)')
	...
	var cacheHtmlElements = function (fileName, line_x) {
		...
		if (self.documents[fileName]['lines'][line_x]['syntax'] == "html") {
			...
			try {
				...
				debug('cacheHtmlElements: while (complete == false) {')
				while (complete == false) {
					...
					//parse string
					debug('cacheHtmlElements: while (charX < text.length) {')
					while (charX < text.length) {
						...
						if (inElement == false && inAttribute == false && text[charX] == '<' && /[a-zA-Z]/.test(text[charX + 1])) {
							...
							charX++;
							//get tag characters
							debug('cacheHtmlElements: while (/[a-zA-Z0-9_.:-]/.test(text[charX]) == true) {')
						...
						if (inElement == true && inAttribute == false) {
							...
							if (/[a-zA-Z]/.test(text[charX]) == true) {
								...
								debug('cacheHtmlElements: while (/[ =><]/.test(text[charX]) == false) {')
								...
								debug('cacheHtmlElements: while (text[charX] != "=") {')
								while (text[charX] != "=") {
									if (/[a-zA-Z<>]/.test(text[charX]) == true) {
										valid = false;
										break;
								...
								debug('cacheHtmlElements: while (text[charX] != "\'" && text[charX] != \'"\') {')
				...
				debug('cacheHtmlElements: while (line_x != lineNumber) {')
	...
	var applyLineDecorations = function (fileName, line_x) {
		...
		if (self.documents[fileName]['decorationEnabled'] == false) {
			return;
		}
	...
	var cacheDocumentLines = function () {
		...
		self.documents[fileName]['nextLineId'] = 0;
		debug('cacheDocumentLines: for (var line_x in lines) {')
		self.fullCache = true;
		...
		debug('cacheDocumentLines: for (var line_x in self.documents[fileName][\'lines\']) {')
	...
	var updateCacheDocumentLines = function (startLine, endLine) {
		debug('updateCacheDocumentLines : ' + startLine + ': ' + endLine)
		...
		try {
			...
			if (diff == 0 && startLine == endLine) {
				//inline change
				let updatedLines = updateLinkedLines(startLine);
				if (updatedLines.length > 0) {
					for (var i in updatedLines) {
						changedLines.push(updatedLines[i]);
					}
				}
				...
				for (var line_x in lines) {
					...
					if (line_x < startLine) {
						...
						if (line_x <= (endLine + diff)) {
							let updatedLines = updateLinkedLines(line_x);
							if (updatedLines.length > 0) {
								for (var i in updatedLines) {
									changedLines.push(updatedLines[i]);
								}
			...
			try {
				debug('updateCacheDocumentLines : for (var i in changedLines) {')
	...
	this.getDocumentLinesInfo = function () {
		...
		if (self.documents[fileName]['cacheEnabled'] == true && self.documentIsValidType(document) && isset(self.documents[fileName]['lines']) == true) {
	...
	*/
	...
	this.setDocumentCacheEnabled = function (params) {
		if (is_object(params)) {
			for (var i in params) {
				documentCacheEnabledParams[i] = params[i];
			}
		} else {
			documentCacheEnabled = is_bool(params) ? params : true;
		}
	}
	...
	this.documentIsValidType = function (document) {
		...
		if (isset(validDocTypes[Helpers.documentFileType(document)]) == false) {
			return false;
		}
		return true;
	}
	...
	var documentOpen = function (document) {
		...
		if (decorations.length > 0) {
			self.documents[fileName]['cacheEnabled'] = true; //on by default when decorations are set
			for (var i in decorations) {
				self.documents[fileName]['decorationEnabled'] = true;
				self.documents[fileName]['decorations'][decorations[i]['key']] = {
		...
		//overrides
		if (isset(documentCacheEnabledParams.lineCount)) {
			self.documents[fileName]['cacheEnabled'] = document.lineCount >= documentCacheEnabledParams.lineCount
		}
	...
	var textDocumentChange = function () {
		try {
			if (textDocumentChangeQueue.length > 0) {
				...
				debug('textDocumentChange: while (true) {')
				while (true) {
				...
				event.diff = diff;
				if (isset(documentCacheEnabledParams.lineCount)) {
					self.documents[fileName]['cacheEnabled'] = self.documents[fileName]['lineCount'] >= documentCacheEnabledParams.lineCount
				}

				if (self.documents[fileName]['cacheEnabled'] == true) {
	...
	vscode.workspace.onDidChangeTextDocument(function (event) {
		...
		if (document.uri.scheme == 'file') {
			if (event.contentChanges.length > 0) {
				debug('textDocumentChange: start')
