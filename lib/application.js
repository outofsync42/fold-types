	...
	var cacheDocumentLines = function () {
		...
		for (var line_x in lines) {
			...
			for (var char_x in line) {
				...
				//block comment
				if (match_lock == "" || match_lock == "in_comment_block" || match_lock == "in_comment_block_end") {
					if (match_lock == "" && line.indexOf('/*', char_x) == char_x) {
						if (line.indexOf('*/') > -1) {
							//same line;
						} else {
							match_length = 2;
					...
					if (match_lock == "in_comment_block" && line.indexOf('*/', char_x) == char_x) {

						match_lock = "in_comment_block_end";
						continue;
					}
					if (match_lock == "in_comment_block_end") {
						match_lock = "";
						continue;
					}
					if (match_lock == "in_comment_block") {
						if (match_length > 0) {
							match_length--;
						} else {
							continue;
			...
			documentLinesInfo[line_x]['lineType'] = lineType;
