...
var Application = function (context) {
	...
	this.getDocumentLinesInfo = function () {
		var fileName = self.document().fileName;
		if (documentCacheEnabled.indexOf(self.documentType()) > -1 && isset(self.documents[fileName]['lines']) == true) {
	...
	let documentCacheEnabled = [];
	this.enableDocumentCache = function (fileTypes) {
		documentCacheEnabled = fileTypes;
	}
	...
	var textDocumentChange = function () {
		try {
			if (textDocumentChangeQueue.length > 0) {
				...
				if (documentCacheEnabled.indexOf(fileType) > -1) {
	...
	vscode.workspace.onDidChangeTextDocument(function (event) {
		...
		if (self.eventIsRegistered('documentTextChange') == false) {
