...
var Application = function (context) {
	...
	var buildDocumentLines = function () {
		...
		//if no document lines are cached
		//cache all lines
		//if document lines already cached
		...
		for (var line_x in lines) {
			...
			for (var char_x in line) {
				...
				if (cache.php_open == false && fileType=='php') {
		...
		return documentLinesInfo;
	}

	this.getDocumentLinesInfo = function () {
		var fileName = self.document().fileName;
		let lines = [];
		if (isset(self.documents[fileName]['lines']) == false) {
			lines = buildDocumentLines();
			if (cacheEnabled && lines !== null) {
				self.documents[fileName]['lines'] = lines;
			}
		} else {
			lines = self.documents[fileName]['lines'];
		}
		return lines;
	}

	var cacheEnabled = false;
	this.cacheDocumentInfo = function () {
		cacheEnabled = true;
	}
	...
	var documentOpen = function (document) {
		if (document.uri.scheme === 'file') {
			...
			if (cacheEnabled) {
				self.getDocumentLinesInfo(); //build initial lines cache
			}
			setTimeout(function () {
	...
	vscode.workspace.onDidOpenTextDocument(function (document) {
		if (document.uri.scheme === 'file' && isset(self.documents[document.fileName]) == false) {
			documentOpen(document)
		}
	});
	vscode.workspace.onDidCloseTextDocument(function (document) {
		if (document.uri.scheme == 'file' && isset(self.documents[document.fileName]) == true) {
	...
	vscode.workspace.onDidChangeTextDocument(function (e) {
		if (e.contentChanges.length > 0 && isset(self.documents[e.document.fileName]) == true) {
